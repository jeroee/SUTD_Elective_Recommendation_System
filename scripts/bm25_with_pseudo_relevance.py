...
def bm25_pseudo_relevance_back(query, tf, tf_norm, idf, vocab, avg_doc_len, k=10):
    query = process_query(query) # lemitize query
    query = expand_query(query,glove_kv,topn=3)  # expand query by including words from pretrianed w2v corpus
...
if __name__ == '__main__':
    ...
    result, ls = bm25_pseudo_relevance_back(query=query, tf=tf, tf_norm=tf_norm, idf=idf, vocab=vocab, avg_doc_len=avg_doc_len, k=10)
